...
from cvc.programs import _build_analysis_prompt, _parse_analysis
...
class TestBuildAnalysisPrompt:
    ...
    def test_includes_gear_flags(self):
        prompt = _build_analysis_prompt(
            _make_context(aligner=True, scrambler=False, miner=True)
        )
    ...
    def test_includes_team_roles(self):
        prompt = _build_analysis_prompt(
            _make_context(roles="miner=4, aligner=2, scrambler=2")
        )
        assert "miner=4" in prompt

    def test_includes_junction_counts(self):
        prompt = _build_analysis_prompt(
            _make_context(junctions={"friendly": 7, "enemy": 3, "neutral": 0})
        )
    ...
    def test_includes_stalled_and_oscillating(self):
        prompt = _build_analysis_prompt(
            _make_context(stalled=True, oscillating=True)
        )
