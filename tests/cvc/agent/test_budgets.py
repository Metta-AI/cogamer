...
from cvc.agent.types import _JUNCTION_ALIGN_DISTANCE, _JUNCTION_AOE_RANGE
...
class TestComputePressureBudgets:
    ...
    def test_economy_bootstrap_low_budget(self):
        ...
        a, s = compute_pressure_budgets(
            step=10, min_resource=10, can_refill_hearts=True, objective="economy_bootstrap"
        )
        assert a == 2
        assert s == 0
...
class TestComputeRetreatMargin:
    ...
    def test_in_enemy_aoe_adds_10(self):
        # margin=15+10=25
        assert compute_retreat_margin(
            hp=30, safe_steps=5, in_enemy_aoe=True, near_enemy_territory=False,
    ...
    def test_near_enemy_territory_adds_5(self):
        # margin=15+5=20
        assert compute_retreat_margin(
            hp=25, safe_steps=5, in_enemy_aoe=False, near_enemy_territory=True,
            heart_count=0, resource_cargo=0, has_gear=True, late_game=False, role="miner",
        ) is True  # 25 <= 5+20=25
    ...
    def test_aoe_takes_priority_over_territory(self):
        ...
        result = compute_retreat_margin(
            hp=26, safe_steps=0, in_enemy_aoe=True, near_enemy_territory=True,
            heart_count=0, resource_cargo=0, has_gear=True, late_game=False, role="miner",
        )
    ...
    def test_heart_count_adds_per_heart(self):
        # margin=15 + 3*5=30
        assert compute_retreat_margin(
            hp=30, safe_steps=0, heart_count=3,
            in_enemy_aoe=False, near_enemy_territory=False,
            resource_cargo=0, has_gear=True, late_game=False, role="miner",
        ) is True

    def test_resource_cargo_contribution(self):
        # min(10, 12)//2 = 5 → margin=15+5=20
        assert compute_retreat_margin(
            hp=20, safe_steps=0, resource_cargo=10,
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, has_gear=True, late_game=False, role="miner",
        ) is True

    def test_resource_cargo_capped_at_12(self):
        # min(100, 12)//2 = 6 → margin=15+6=21
        assert compute_retreat_margin(
            hp=21, safe_steps=0, resource_cargo=100,
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, has_gear=True, late_game=False, role="miner",
        ) is True
        assert compute_retreat_margin(
            hp=22, safe_steps=0, resource_cargo=100,
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, has_gear=True, late_game=False, role="miner",
        ) is False
    ...
    def test_no_gear_adds_10(self):
        # margin=15+10=25
        assert compute_retreat_margin(
            hp=25, safe_steps=0, has_gear=False,
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, resource_cargo=0, late_game=False, role="miner",
        ) is True

    def test_late_game_aligner_adds_10(self):
        # margin=15+10=25
        assert compute_retreat_margin(
            hp=25, safe_steps=0, late_game=True, role="aligner",
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, resource_cargo=0, has_gear=True,
        ) is True

    def test_late_game_scrambler_adds_10(self):
        assert compute_retreat_margin(
            hp=25, safe_steps=0, late_game=True, role="scrambler",
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, resource_cargo=0, has_gear=True,
        ) is True
    ...
    def test_late_game_miner_adds_5(self):
        # margin=15+5=20
        assert compute_retreat_margin(
            hp=20, safe_steps=0, late_game=True, role="miner",
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, resource_cargo=0, has_gear=True,
        ) is True
        assert compute_retreat_margin(
            hp=21, safe_steps=0, late_game=True, role="miner",
            in_enemy_aoe=False, near_enemy_territory=False,
            heart_count=0, resource_cargo=0, has_gear=True,
        ) is False
    ...
    def test_all_modifiers_combined(self):
        ...
        assert compute_retreat_margin(
            hp=71, safe_steps=10, in_enemy_aoe=True, near_enemy_territory=True,
            heart_count=2, resource_cargo=12, has_gear=False, late_game=True, role="aligner",
        ) is True  # 71 <= 10+61=71
        assert compute_retreat_margin(
            hp=72, safe_steps=10, in_enemy_aoe=True, near_enemy_territory=True,
            heart_count=2, resource_cargo=12, has_gear=False, late_game=True, role="aligner",
        ) is False
...
class TestComputePressureMetrics:
    ...
    def test_best_frontier_coverage(self, make_entity):
        ...
        unreachable1 = make_entity(x=20, y=0)  # dist 20 from source (out), dist 10 from frontier (in)
        unreachable2 = make_entity(x=10, y=14)  # dist 14 from source (in range!) -- actually frontier
