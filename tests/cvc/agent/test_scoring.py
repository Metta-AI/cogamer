...
class TestIsClaimedByOther:
    def test_no_claim(self):
        assert (
            is_claimed_by_other(claims={}, candidate=(10, 10), agent_id=0, step=100)
            is False
        )
    ...
    def test_claimed_by_self(self):
        claims = {(10, 10): (0, 100)}
        assert (
            is_claimed_by_other(claims=claims, candidate=(10, 10), agent_id=0, step=100)
            is False
        )
    ...
    def test_claimed_by_other_recent(self):
        ...
        assert (
            is_claimed_by_other(
                claims=claims, candidate=(10, 10), agent_id=0, step=100 + _TARGET_CLAIM_STEPS
            )
            is True
        )
    ...
    def test_claimed_by_other_expired(self):
        ...
        assert (
            is_claimed_by_other(
                claims=claims, candidate=(10, 10), agent_id=0, step=100 + _TARGET_CLAIM_STEPS + 1
            )
            is False
        )
    ...
    def test_different_candidate_not_claimed(self):
        claims = {(10, 10): (1, 100)}
        assert (
            is_claimed_by_other(claims=claims, candidate=(20, 20), agent_id=0, step=100)
            is False
        )
...
class TestIsUsableRecentExtractor:
    ...
    def test_old_extractor(self, make_entity):
        entity = make_entity(entity_type="extractor", last_seen_step=100)
        assert (
            is_usable_recent_extractor(entity, step=100 + _EXTRACTOR_MEMORY_STEPS + 1) is False
        )
...
class TestScrambleTargetScore:
    ...
    def test_threat_bonus_from_friendly_junctions(self, make_entity):
        candidate = make_entity(entity_type="junction", x=50, y=50)
        friendly = make_entity(
            entity_type="junction", x=50 + _JUNCTION_ALIGN_DISTANCE, y=50
        )
    ...
    def test_friendly_outside_align_distance_no_threat(self, make_entity):
        candidate = make_entity(entity_type="junction", x=50, y=50)
        friendly = make_entity(
            entity_type="junction", x=50 + _JUNCTION_ALIGN_DISTANCE + 1, y=50
        )
        score_with, _ = scramble_target_score(
            current_position=(50, 50),
            hub_position=(50, 50),
